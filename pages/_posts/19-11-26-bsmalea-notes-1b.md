...
In the <a href="{{ site.url }}/bsmalea-notes-1a">previous post</a>, we went over polynomial regression. In the Python implementation, I chose to go with a polynomial of order $4$ (for no reason whatsoever). However, perhaps $M=4$ isn't the 'best' choice - but what is the 'best' choice, and how do we find it? Firstly, the order of our polynomial is set before the training process begins, and we call these special parameters in our model **"hyperparameters"**. Secondly, the process of figuring out the values of the hyperparameters is called **hyperparameter optimization** and is a part of **model selection**. Thirdly, as mentioned in the <a href="{{ site.url }}/bsmalea-notes-1a">previous post</a>, machine learning is mostly concerned with prediction, which means that we define the 'best' model as the one that **generalizes** the best, i.e. which model would perform the best on data it wasn't trained on?
